...
import { Clock, Flame, ArrowRight, Zap, MapPin, RefreshCw } from 'lucide-react'
...
const typeLabel: Record<string, { label: string; color: string }> = {
  ...
  attraction: { label: 'Attraction', color: 'text-sky-700 bg-sky-50 border border-sky-100' },
  restaurant: { label: 'Dining', color: 'text-rose-700 bg-rose-50 border border-rose-100' },
  transfer: { label: 'Transfer', color: 'text-slate-700 bg-slate-50 border border-slate-100' },
}
...
function DealCard({ deal, idx }: { deal: Deal; idx: number }) {
  const tl = typeLabel[deal.type] ?? typeLabel.package
...
export default function DealsSection() {
  const { deals, isLoading, error, refetch } = useDeals({ limit: 6, active: true })
      ...
      <div className='max-w-[1240px] mx-auto px-4 sm:px-6'>
        ...
        {error && (
          <div className='flex flex-col items-center justify-center py-16 text-slate-400'>
            <p className='text-sm mb-4'>Could not load deals right now.</p>
            <button
              onClick={refetch}
              className='flex items-center gap-2 text-xs font-semibold text-red-500 border border-red-200 px-4 py-2 rounded-lg hover:bg-red-50 transition-colors'
            >
        ...
        {!error && (
          <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5'>
            {isLoading
              ...
              : deals.map((deal, idx) => <DealCard key={deal.id} deal={deal} idx={idx} />)
            }
          </div>
        )}
